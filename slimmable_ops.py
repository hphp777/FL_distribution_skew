...
class USConv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        #Dialation: the spacing between kernel elements. Can be a single number or a tuple (dH, dW). Default: 1
        self.groups = in_channels if self.depthwise else 1 # Split input into groups, in_channels should be divisible by the number of groups
        weight = self.weight[:out_channels, :in_channels, :, :] # filters of shape(out_channels, in_channels/group, kH, kW)
        if self.bias is not None:
            bias = self.bias[:out_channels]
        else:
            bias = self.bias
        # torch.nn.Conv2D와의 차이점: weight를 직접 선언한다. (외부에서 만든 필터를 사용)
