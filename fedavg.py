...
class server():
    ...
    def new_merge_weight(self, weights, client_num, clients):

        weight = self.model.state_dict()
        ci = []
        
        for i in range(client_num):
            ci.append(clients[i].dataloader.count_imbalance(i))
        e_x = np.exp(ci - ci.max())
        e_x = e_x / e_x.sum()

        for key in weight:
            weight[key] = sum([weights[i][key] * e_x[i] for i in range(client_num)]) 
    ...
    def test(self, weight, round):
        ...
        with torch.no_grad(): # for the evaluation mode
            ...
            for _, (imgs, labels) in enumerate(tqdm(dataloader, desc= "Test Round")):
                ...
                total += len(labels)
            acc = float(test_correct) / float(total)

            print("Global Test AUC: ",roc_auc_score(y_true = all_labels, y_score = all_preds, multi_class='ovr'))
...
class client():
    ...
    def train(self,updated = False, weight = None, t_r = None):
        ...
        for epoch in range(self.epochs):
            ...
            for _, (imgs, labels) in enumerate(tqdm(self.dataloader, desc="Training Epoch " + str(epoch+1) + "/" + str(self.epochs))):
                ...
                teacher_output = self.model(imgs) # [B x 32 x 16 x 16, B x 64 x 8 x 8], B x num_classes
            ...
            # self.scheduler.step()
            acc = (float(total_correct) /float(total_data)) * 100
